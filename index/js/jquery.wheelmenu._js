...
!function($){
  ...
  var defaults = {
    ...
    this.css("top", buttonY  - (this.outerHeight() / 2 +35)  + "px");
    this.css("left", buttonX - (this.outerWidth() / 2 +35)   + "px");
    return this;
  }
  ...
  $.fn.flyIn = function (el, button, width, height, angle, step, radius, settings) {
    ...
    this.each(function(index) {
      ...
      var x = Math.round(width/2 + radius * Math.cos(angle) - $(this).find("a").outerWidth()/2 +35),
          y = Math.round(height/2 + radius * Math.sin(angle) - $(this).find("a").outerHeight()/2 +35);
